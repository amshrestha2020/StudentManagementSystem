...
from student_management_app.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, LeaveReportStaff, Staffs, FeedBackStaffs, CustomUser, Courses, NotificationStaffs, StudentResult, OnlineClassRoom
...
def staff_home(request):
    ...
    leave_count                  =  LeaveReportStaff.object.filter(staff_id = staff.id, leave_status=1).count()
...
def start_live_classroom_process(request):
    ...
    if checks:
        ...
        staff_obj             =  Staffs.objects.get(admin=request.user.id)
        onlineClass           =  OnlineClassRoom(room_name=roomname,
                                                room_pwd=room_pwd,
                                                subject=subject_obj,
    ...
    return render(request,"staff_template/live_class_room_start.html",{"username":request.user.username,
                                                                        "password":room_pwd,
                                                                        "roomid":roomname,
